    ...
    <>
      ...
      >
        <div className="popup-box">
          <div className="popup-header">
            <Radio.Group type="button" value={data.pay_type} onChange={(value) => setData({...data, pay_type: value})}>
              <Radio value={1}>支出</Radio>
              <Radio value={2}>收入</Radio>
            </Radio.Group>

            <div className="date" onClick={() => setDateVisible(true)}>{currentDate}</div>
          </div>
          {/* 分类选择 */}
          <TypeList onChange={(type) => {
            setData({
              ...data,
              ...type
            })
            console.log(type)
          }} data={typeList} type={data.pay_type}></TypeList>
          <div className="price">
            <div className="unit">
        ...
        {/* 虚拟键盘 */}
        <PriceKeyboard onClick={(value) => setData({...data, amount: +value})} onConfirm={confirmHandler}></PriceKeyboard>
      </Popup>
      ...
      <DatePicker 
        ...
        onConfirm={(date) => {
          ...
          // date.getTime() 是要设置到表单上给后台的
          // date是一个日期对象，是某年某月某日零点零分零秒的日期对象
          const now = new Date() // 获取当前日期对象now.getHours() now.getMinutes()
          ...
          setCurrentDate(moment(date.getTime()).format('YYYY/MM/DD'))
